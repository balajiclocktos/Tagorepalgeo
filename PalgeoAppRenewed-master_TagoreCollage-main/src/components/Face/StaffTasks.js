...
export default class StaffTasks extends Component {
  constructor(props) {
    super(props);
    this.state = {
      loader: true,
  ...
  goToCheckout = () => {
    const {travel_check_in, qrCheckin} = this.props.route.params;
    var success = false;
      ...
      fetch(Const + 'api/Staff/UpdateStaffTasksAttended', {
        ...
        .then(json => {
          ...
          if (json) {
            if (travel_check_in === 'no' && qrCheckin === 'false') {
              this.props.navigation.navigate('Checkout', {show: true});
            } else {
            ...
            if (Platform.OS === 'ios') {
              ...
              return alert('Tasks not saved.Please try later');
            }
            this.setState({
              showAlert1: true,
        ...
        .catch(error => {
          ...
          this.setState({
            showAlert1: true,
            error_message:
              'You might not be connected to the Internet. Please check your connection and try again.',
            showAlert: false,
          });
        });
    } else {
  ...
  render() {
    ...
    //       <View style={{flex:1}}>
    //
    //               <SubHeader title="Staff Tasks"  showBack={true}  backScreen="Home" showBell={false} navigation={this.props.navigation}/>
